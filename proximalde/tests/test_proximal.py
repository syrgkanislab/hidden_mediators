...
from ..gen_synthetic_data import gen_data_w_controls, gen_data_no_controls, gen_data_no_controls_discrete_m
...
def test_violations():
    np.random.seed(123)
    n = 10000
    W, D, _, Z, X, Y = gen_data_no_controls(n, pz=1, px=1, a=1., b=1., c=.5, d=0.0, e=0.0, f=1.0, g=0.0)
    ...
    np.random.seed(1234)
    _, D, _, Z, X, Y = gen_data_no_controls(n, 1, 1, 1., 1., .5, 0.0, 1.0, 1.0, 0.0)
    D = D - D.mean(axis=0, keepdims=True)
    Z = Z - Z.mean(axis=0, keepdims=True)
    X = X - X.mean(axis=0, keepdims=True)
    _, _, _, _, _, _, pval, dval, strength, strength_std, *_ = estimate_nuisances(D, Z, X, Y,
    ...
    np.random.seed(123)
    _, D, _, Z, X, Y = gen_data_no_controls(n, 1, 1, 1., 1., .5, 1.0, 0.0, 1.0, 0.0)
    D = D - D.mean(axis=0, keepdims=True)
    Z = Z - Z.mean(axis=0, keepdims=True)
    X = X - X.mean(axis=0, keepdims=True)
    _, _, _, _, _, _, pval, dval, strength, strength_std, *_ = estimate_nuisances(D, Z, X, Y,
    ...
    np.random.seed(123)
    _, D, _, Z, X, Y = gen_data_no_controls(n, 1, 1, 1., 1., .5, 0.0, 1.0, 0.0, 0.0)
    D = D - D.mean(axis=0, keepdims=True)
    Z = Z - Z.mean(axis=0, keepdims=True)
    X = X - X.mean(axis=0, keepdims=True)
    _, _, _, _, _, _, pval, dval, strength, strength_std, *_ = estimate_nuisances(D, Z, X, Y,
...
def test_raise_nonfitted():
    ...
    for method in [pde.bootstrap_inference, pde.conf_int, pde.run_influence_diagnostics, pde.subsample_all_stages,
...
def test_pde_fit():
    ...
    for semi, cv, ivreg_type, \
        ...
        assert pde.gamma_.shape == (Z.shape[1], 1)
...
def test_pde_run_diagnostics():
    ...
    pde = ProximalDE(cv=2, n_jobs=1)
    pde.fit(W, D, Z, X, Y)
    diag = pde.run_influence_diagnostics()
...
def test_influential_set():
    ...
    for sign in [-1, 1]:
        ...
        with pytest.raises(AttributeError, match="run_influence_diagnostics") as e_info:
            est.influential_set()
        print(e_info)

        est.run_influence_diagnostics()
...
def test_rank_violation_caught():
    ...
    sm, sz, sx, sy = 2, 1, 1, 1
    _, D, _, Z, X, Y = gen_data_no_controls(n, pz, px, a, b, c, d, e, f, g, sm=sm, sz=sz, sx=sx, sy=sy)
    D = D.reshape(-1, 1)
    D = D - D.mean(axis=0)
    X = X - X.mean(axis=0)
    Z = Z - Z.mean(axis=0)
...
def test_accuracy_no_violations():
    ...
    for n in [10000, 100000]:
        for pz, px, pass_w in [(3, 2, True), (2, 3, False)]:
            for ivreg_type,\
                model_regression, model_classification in [('2sls', 'linear', 'linear'),
                                                           ('adv', 'xgb', 'xgb'),
                                                           ]:
                for _ in range(5):
                    print(n, pz, px, ivreg_type)
                    pw = 1
                    # Indirect effect is a*b, direct effect is c
                    a = .7  # (2 * np.random.binomial(1, .5) - 1) * np.random.uniform(.5, 2)
                    b = .8  # (2 * np.random.binomial(1, .5) - 1) * np.random.uniform(.5, 2)
            ...
            for px in [1, 2]:
                for sm in [0.01, 2.0]:
                        for ivreg_type in ['2sls', 'adv']:
                            print('n, pz, px, sm, ivreg_type = ', n, pz, px, sm, ivreg_type)
                            pw = 1
                            # Indirect effect is a*b, direct effect is c
                            a = .7  # (2 * np.random.binomial(1, .5) - 1) * np.random.uniform(.5, 2)
                            b = .8  # (2 * np.random.binomial(1, .5) - 1) * np.random.uniform(.5, 2)
                            ...
                            Y = Y.reshape(-1, 1)
                            Y = Y - Y.mean(axis=0)
                            est = ProximalDE(ivreg_type=ivreg_type, cv=5, semi=True,
                                             n_jobs=-1, random_state=3, verbose=0)
                            est.fit(W, D, Z, X, Y)
                            if pz == 1:
...
def test_pi_and_var_pi():
    ...
    W, D, _, Z, X, Y = gen_data_no_controls(100000, pz, px, a, b, c, d, e, f, g, sm=sm)
    D = D.reshape(-1, 1)
    D = D - D.mean(axis=0)
    X = X - X.mean(axis=0)
    Z = Z - Z.mean(axis=0)
