...
final_ss_dy = pk.load(open('all_dy.pkl', 'rb'))
...
def run_bootstrap(D_label, Y_label, Xset, Zset, save_dir, frac=.5):
    ...
    inf3 = est.bootstrap_inference(stage=3, n_subsamples=1000, fraction=0.75, replace=False, verbose=3, random_state=123)
    inf3.summary(save_dir=save_dir, save_fname_addn=f'_bs_stage3_nBs1000_frac0.75')
...
#d,y = 'On_dis', 'RA'
#d,y = 'Obese', 'OA'
point, _, path, (Xset, Zset) = final_ss_dy[f'{d}_{y}']
